...
public:
    int minCostConnectPoints(vector<vector<int>>& points) {
        ...
        for (int i = 0; i < n; ++i) { // we want to add n nodes into the tree
            ...
            total += weights[u];
            for (int i = 0; i < n; ++i) { // give our updates to the weights as we added new node
                if (in_tree[i]) continue; // already in our tree so no need to update these weights
                int w = abs(points[i][0] - points[u][0]) + abs(points[i][1] - points[u][1]);
                if (weights[i] > w) weights[i] = w;
            }
        }
        return total;
    }
